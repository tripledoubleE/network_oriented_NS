...
class KnowledgeBasedDataset(Dataset):
    ...
    def _merge_item_and_entity(self):
        ...
        #print('_merge_item_and_entity function')
        #print('????????????????????', len(self.field2id_token["HotelId"]))
        item_token = self.field2id_token[self.iid_field]
        #file_path = '/home/ece/Desktop/item_ids.txt'
        #np.savetxt(file_path, item_token, fmt='%s', delimiter=',')
        ...
        for i in entity_order[1 : link_num + 1]:
            #print(i)
            #print(entity_token[i])
            #print(entity_token[i])
            #print('bu neeee ')
            #print(new_item_token2id)
            #print(self.entity2item[entity_token[i]])
            #print(new_item_token2id[self.entity2item[entity_token[i]]])
