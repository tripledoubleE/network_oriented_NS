...
class AbstractDataLoader(torch.utils.data.DataLoader):
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        print("DATASET NEDIR ??? NE DEGILDIR ??? ")
        # user_num = dataset.user_num
        # self.user_list = Interaction({self.uid_field: torch.arange(dataset.user_num)})
        # print(dataset.inter_feat[dataset.uid_field].numpy())
        # print(Interaction({dataset.uid_field: torch.arange(dataset.user_num)}))
        # print(dataset.inter_matrix(form="csr").astype(
        #         np.float32
        #     ))


        # DATASET IN FEATURE LARINI ANLA 
        #   ---> LEN USER'A IHTIYAC VAR ++++
        #   ---> INTERACTION MATRIX'E IHTIYAC VAR, NX GRAPH OLUSTURULACAK !!!!
...
class NegSampleDataLoader(AbstractDataLoader):
    ...
    def _set_neg_sample_args(self, config, dataset, dl_format, neg_sample_args):
        ...
        self.times = 1
        if (
            self.neg_sample_args["distribution"] in ["uniform", "popularity", "powerlaw-1", "powerlaw-2", "mcns", "item_proj", "dens", "dns"]
    ...
    def _neg_sampling(self, inter_feat):
            if self.neg_sample_args.get("dynamic", False):
                ...
                neg_candidate_ids = self._sampler.sample_by_user_ids(
                    user_ids, item_ids, self.neg_sample_num * candidate_num, self.dataset
                )
                self.model.eval()
                ...
                user_ids = inter_feat[self.uid_field].numpy()
                #print('_neg_sampling data loader user id: ', user_ids.shape)
                item_ids = inter_feat[self.iid_field].numpy()
                neg_item_ids = self._sampler.sample_by_user_ids(
                    user_ids, item_ids, self.neg_sample_num, self.dataset
                )
                return self.sampling_func(inter_feat, neg_item_ids)
            else:
                return inter_feat
