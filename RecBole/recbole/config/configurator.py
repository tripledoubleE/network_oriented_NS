...
class Config(object):
    ...
    def _set_train_neg_sample_args(self):
        ...
        if train_neg_sample_args is None or train_neg_sample_args == "None":
            ...
            elif distribution not in ["uniform", "popularity", "powerlaw-1", "powerlaw-2", "mcns", "item_proj", "dens", "dns"]:
                raise ValueError(
                    f"The distribution [{distribution}] of train_neg_sample_args "
                    f"should in ['uniform', 'popularity', 'powerlaw-1', 'powerlaw-2', 'mcns', 'item_proj', 'dens', 'dns]"
                )
    ...
    def _set_eval_neg_sample_args(self, phase: Literal["valid", "test"]):
        ...
        elif eval_mode[0:7] == "power1-":
            sample_num = int(eval_mode[7:])
            eval_neg_sample_args = {"distribution": "powerlaw-1", "sample_num": sample_num}
        elif eval_mode[0:7] == "power2-":
            sample_num = int(eval_mode[7:])
            eval_neg_sample_args = {"distribution": "powerlaw-2","sample_num": sample_num}
        elif eval_mode[0:4] == "mcns":
            sample_num = int(eval_mode[5:])
            eval_neg_sample_args = {"distribution": "mcns","sample_num": sample_num}
        elif eval_mode[0:9] == "item_proj":
            sample_num = int(eval_mode[9:])
            eval_neg_sample_args = {"distribution": "item_proj","sample_num": sample_num}
        elif eval_mode[0:4] == "dens":
            sample_num = int(eval_mode[4:])
            eval_neg_sample_args = {"distribution": "dens","sample_num": sample_num}
        elif eval_mode[0:3] == "dns":
            sample_num = int(eval_mode[3:])
            eval_neg_sample_args = {"distribution": "dens","sample_num": sample_num}
        else:
