...
'''
    ...
    if not test_data.is_sequential:
        ...
        history_item = test_data.dataset.uid2history_item[list(uid_series)]
...
def full_sort_scores(uid_series, model, test_data, device=None):
    ...
    if not test_data.is_sequential:
        ...
        input_interaction.update(dataset.get_item_feature().repeat(len(uid_series)))
        ...
        input_interaction = dataset[index]
        score = model.full_sort_predict(input_interaction)
